...
describe("useBookingStore Business Logic", () => {
  ...
  describe("Product and Package Selection", () => {
    it("should add a standalone product", () => {
      ...
      const state = get();
      expect(state.selectedProducts.some((p) => p.productId === "p2")).toBe(
        true,
      );
    });
    ...
    it("should select a product within a package, replacing previous selection", () => {
      ...
      let state = get();
      expect(state.packageSelections.get(foodPackageKey)?.productId).toBe(
        cateringId,
      );
      expect(
        state.selectedProducts.some((p) => p.productId === cateringId),
      ).toBe(true);
      ...
      expect(state.selectedProducts.some((p) => p.productId === pizzaId)).toBe(
        true,
      );
    });
  });
  ...
  describe("Price Calculation", () => {
    it("should calculate price for an evening event with per-person products", () => {
      ...
      const expectedCatering =
        mockProducts.find((p) => p._id === "p3")!.price * 20; // 75 * 20 = 1500
    ...
    it("should add per-hour products correctly", () => {
      ...
      const expectedExtraHours =
        mockProducts.find((p) => p._id === "p4")!.price * 3; // 200 * 3 = 600
      expect(totalPrice).toBe(expectedBase + expectedExtraHours); // 1500 + 600 = 2100
    });

    it("should allow selecting a start time and reset on slot/date change", () => {
      get().setSelectedTimeSlot("evening");
      get().setSelectedStartTime("20:00");
      expect(get().selectedStartTime).toBe("20:00");
      get().setSelectedTimeSlot("afternoon");
      expect(get().selectedStartTime).toBeNull();
    });

    it("should set and reset extra hours correctly", () => {
      get().setExtraHours(2);
      expect(get().extraHours).toBe(2);
      get().setExtraHours(0);
      expect(get().extraHours).toBe(0);
    });
  });
