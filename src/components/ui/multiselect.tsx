    ...
    >
      ...
      >
        <div className="flex flex-wrap gap-1">
          {selected.map((option) => {
            const colorClass =
              ROLE_COLORS[option.value] ||
              "bg-gray-100 text-gray-800 border-gray-300";
            const label = ROLE_LABELS[option.value] || option.label;
            return (
              <div
                key={option.value}
                className={cn(
                  "animate-fadeIn relative inline-flex h-7 cursor-default items-center rounded-md border ps-2 pe-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pe-2 gap-x-1",
                  colorClass,
                  badgeClassName,
                )}
              ...
              >
                {label}
                <button
                  className="text-muted-foreground/80 hover:text-foreground focus-visible:border-ring focus-visible:ring-ring/50 -inset-y-px -end-px flex size-7 items-center justify-center rounded-e-md border border-transparent p-0 outline-hidden transition-[color,box-shadow] outline-none focus-visible:ring-[3px]"
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
      ...
      <div className="relative">
        {createPortal(
          <div
            className={cn(
              "border-input absolute top-2 z-[9999] w-fit overflow-hidden rounded-md border",
